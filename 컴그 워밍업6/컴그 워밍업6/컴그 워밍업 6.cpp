...
// 랜덤 방향을 선택하는 함수 (0: 우, 1: 하, 2: 좌, 3: 상)
int randomDirection(int currentDirection) {
    int newDirection = rand() % 4;

    // 같은 방향으로 계속 가지 않게 함
    while ((currentDirection == 0 && newDirection == 2) ||
        (currentDirection == 1 && newDirection == 3) ||
        (currentDirection == 2 && newDirection == 0) ||
        (currentDirection == 3 && newDirection == 1)) {
        newDirection = rand() % 4;
    }
...
void generatePath() {
    int x = 0, y = 0;
    int direction = rand() % 4; // 0: 우, 1: 하, 2: 좌, 3: 상
    ...
    while (x != SIZE - 1 || y != SIZE - 1) {
        if (stepCount >= rand() % MAX_STEPS_IN_ONE_DIRECTION + 1) {
            // 무작위로 1~5칸 사이의 값으로 이동 후 방향 전환
